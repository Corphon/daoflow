...
type CouplingState struct {
	...
	Properties CouplingProperties // 耦合属性
	Quantum    QuantumProperties  // 量子特性
	Dynamics   DynamicProperties  // 动态特性
}

// CouplingProperties 耦合属性
...
type QuantumProperties struct {
	...
	Correlation  float64 // 量子关联度
}

// DynamicProperties 动态特性
...
// initSpacetime 初始化时空特性
func (fc *FieldCoupling) initSpacetime() error {
	// 计算初始场间距离
...
// normalizeValue 标准化值到[0,1]范围
func normalizeValue(value float64) float64 {
	return math.Max(0, math.Min(1, value))
}

// calculateNorm 计算场的范数
func (fc *FieldCoupling) calculateNorm(field *FieldTensor) float64 {
	norm := 0.0
	for i := 0; i < field.dimension; i++ {
		for j := 0; j < field.dimension; j++ {
			val, _ := field.GetComponent([]int{i, j})
			norm += cmplx.Abs(val) * cmplx.Abs(val)
		}
	}
	return math.Sqrt(norm)
}

// calculateFieldOverlap 计算场重叠
func (fc *FieldCoupling) calculateFieldOverlap() (float64, error) {
	...
	for i := 0; i < dim; i++ {
		for j := 0; j < dim; j++ {
			val1, _ := fc.field1.GetComponent([]int{i, j})
			val2, _ := fc.field2.GetComponent([]int{i, j})
			// 使用复数绝对值的乘积
			overlap += cmplx.Abs(val1) * cmplx.Abs(val2)
		}
	}
...
// calculateStateOverlap 计算量子态重叠
func (fc *FieldCoupling) calculateStateOverlap() float64 {
	// 使用DotProduct方法计算量子态重叠
	overlap, err := fc.field1.quantum.state.DotProduct(fc.field2.quantum.state)
	if err != nil {
		return 0.0 // 如果计算内积失败，返回0重叠
	}

	// 返回重叠度的模
	return cmplx.Abs(overlap)
}

func (fc *FieldCoupling) calculatePhase() (float64, error) {
	phase1 := fc.field1.quantum.state.GetPhase()
	phase2 := fc.field2.quantum.state.GetPhase()
...
func (fc *FieldCoupling) calculateStability() (float64, error) {
	...
	for i := last; i > max(0, last-10); i-- {
		...
		strengthVar += math.Pow(curr.Properties.Strength-prev.Properties.Strength, 2)
		energyVar += math.Pow(curr.Properties.Energy-prev.Properties.Energy, 2)
	}
...
func (fc *FieldCoupling) calculateDistance() (float64, error) {
	...
	for i := 0; i < dim; i++ {
		for j := 0; j < dim; j++ {
			...
			distance += cmplx.Abs(diff) * cmplx.Abs(diff)
		}
	}
...
// calculateLinearRegression 计算线性回归斜率
