...
func (es *EnergySystem) GetBalance() float64 {
	...
	expectedRatio := 1.0 / float64(len(energyTypes))
	variance := 0.0

	for _, eType := range energyTypes {
		ratio := es.getEnergy(eType) / total
		diff := ratio - expectedRatio
		variance += diff * diff
	}

	variance /= float64(len(energyTypes))
