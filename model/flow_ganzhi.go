...
type StemState struct {
	Energy        float64
	Phase         Phase
	WuXingElement WuXingElement // 关联五行
...
func (f *GanZhiFlow) initializeStates() {
	...
	for i := Jia; i <= Gui; i++ {
		f.state.stems[i] = &StemState{
			...
			Phase:         PhaseNone,
			WuXingElement: f.getStemWuXingElement(i),
	...
	for i := Zi; i <= Hai; i++ {
		f.state.branches[i] = &BranchState{
			...
			Phase:         PhaseNone,
			WuXingElement: f.getBranchWuXingElement(i),
...
func (f *GanZhiFlow) Transform(pattern TransformPattern) error {
	...
	case PatternBalance:
		return f.harmonizeWuXingElements()
	default:
...
func (f *GanZhiFlow) updateEnergies(stem HeavenlyStem, branch EarthlyBranch) error {
	...
	// 基于五行相生相克关系调整能量
	WuXingElementInteraction := f.calculateWuXingElementInteraction(stemState.WuXingElement, branchState.WuXingElement)

	// 基于阴阳属性调整能量
	polarityFactor := f.calculatePolarityFactor(stemState.Polarity, branchState.Polarity)

	// 计算新能量
	newStemEnergy := stemEnergy * (1 + WuXingElementInteraction*polarityFactor)
	newBranchEnergy := branchEnergy * (1 + WuXingElementInteraction*polarityFactor)
...
func (f *GanZhiFlow) updateHarmony() error {
	...
	// 计算五行和谐度
	WuXingElementHarmony := f.calculateWuXingElementHarmony()
	f.components.harmonizer.UpdateComponent("WuXingElement", WuXingElementHarmony)
...
// calculateWuXingElementInteraction 计算五行相互作用
func (f *GanZhiFlow) calculateWuXingElementInteraction(elem1, elem2 WuXingElement) float64 {
	// 五行相生关系：+0.2
	// 五行相克关系：-0.1
	// 五行相同：+0.1
	// 其他情况：0
...
// calculateWuXingElementHarmony 计算五行和谐度
func (f *GanZhiFlow) calculateWuXingElementHarmony() float64 {
	WuXingElementCount := make(map[WuXingElement]float64)
	totalEnergy := 0.0

	// 统计各五行能量
	for _, state := range f.state.stems {
		WuXingElementCount[state.WuXingElement] += state.Energy
	...
	for _, energy := range WuXingElementCount {
...
// 辅助函数：判断五行相生
func isGenerating(e1, e2 WuXingElement) bool {
	// 木生火、火生土、土生金、金生水、水生木
	generatingPairs := map[WuXingElement]WuXingElement{
		Wood:  Fire,
		Fire:  Earth,
		Earth: Metal,
		Metal: Water,
...
// 辅助函数：判断五行相克
func isControlling(e1, e2 WuXingElement) bool {
	// 木克土、土克水、水克火、火克金、金克木
	controllingPairs := map[WuXingElement]WuXingElement{
		Wood:  Earth,
		Earth: Water,
		Water: Fire,
		Fire:  Metal,
...
// balanceWuXingElements 平衡五行能量
func (f *GanZhiFlow) balanceWuXingElements(distribution map[WuXingElement]float64) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	...
	// 计算每个五行的能量差异
	adjustments := make(map[WuXingElement]float64)
	for WuXingElement, energy := range distribution {
		adjustments[WuXingElement] = idealEnergy - energy
	}

	// 应用调整到天干
	for stem, state := range f.state.stems {
		WuXingElement := state.WuXingElement
		adjustment := adjustments[WuXingElement] / float64(len(f.state.stems))
	...
	for branch, state := range f.state.branches {
		WuXingElement := state.WuXingElement
		adjustment := adjustments[WuXingElement] / float64(len(f.state.branches))
...
// harmonizeWuXingElements 调和五行
func (f *GanZhiFlow) harmonizeWuXingElements() error {
	// 计算五行分布
	WuXingElementDistribution := make(map[WuXingElement]float64)

	// 统计天干五行能量
	for _, state := range f.state.stems {
		WuXingElementDistribution[state.WuXingElement] += state.Energy
	}
	...
	// 调和五行能量
	if err := f.balanceWuXingElements(WuXingElementDistribution); err != nil {
		return err
	}
...
func (f *GanZhiFlow) calculateInteractions(stem HeavenlyStem, branch EarthlyBranch) error {
	...
	// 计算五行相互作用
	WuXingElementEffect := f.calculateWuXingElementInteraction(stemState.WuXingElement, branchState.WuXingElement)

	// 计算阴阳相互作用
	polarityEffect := f.calculatePolarityFactor(stemState.Polarity, branchState.Polarity)

	// 复合作用系数
	interactionFactor := WuXingElementEffect * polarityEffect
...
func (f *GanZhiFlow) calculateRelationFactor(stemState *StemState, branchState *BranchState) float64 {
	...
	// 考虑五行相生相克
	WuXingElementFactor := f.calculateWuXingElementInteraction(stemState.WuXingElement, branchState.WuXingElement)
	baseStrength += WuXingElementFactor * 0.4 // 五行影响权重 40%
...
// 辅助方法...
// getStemWuXingElement 获取天干五行属性
func (f *GanZhiFlow) getStemWuXingElement(stem HeavenlyStem) WuXingElement {
	switch stem {
	case Jia, Yi: // 甲乙
		return Wood
	case Bing, Ding: // 丙丁
...
// getBranchWuXingElement 获取地支五行属性
func (f *GanZhiFlow) getBranchWuXingElement(branch EarthlyBranch) WuXingElement {
	switch branch {
	case Yin, Mao: // 寅卯
		return Wood
	case Si, Wu_Branch: // 巳午
		return Fire
	case Chen, Xu, Chou, Wei: // 辰戌丑未
		return Earth
	case Shen, You: // 申酉
		return Metal
	case Hai, Zi: // 亥子
...
// getStemPolarity 获取天干阴阳属性
func (f *GanZhiFlow) getStemPolarity(stem HeavenlyStem) Nature {
	switch stem {
	case Jia, Bing, Wu, Geng, Ren: // 甲丙戊庚壬
		return NatureYang
	case Yi, Ding, Ji, Xin, Gui: // 乙丁己辛癸
		return NatureYin
	default:
...
// getBranchPolarity 获取地支阴阳属性
func (f *GanZhiFlow) getBranchPolarity(branch EarthlyBranch) Nature {
	switch branch {
	case Zi, Yin, Chen, Wu_Branch, Shen, Xu: // 子寅辰午申戌
		return NatureYang
	case Chou, Mao, Si, Wei, You, Hai: // 丑卯巳未酉亥
		return NatureYin
	default:
		return NatureNeutral
	}
